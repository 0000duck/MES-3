...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormProject: Office2007Form
    {
        private SpareEntities _db = EntitiesFactory.CreateSpareInstance();
        public FormProject()
        {
            InitializeComponent();
            bs.DataSource = _db.TA_PROJECT.ToList();
...
//            grid.PrimaryGrid.ColumnAutoSizeMode = ColumnAutoSizeMode.Fill;
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_PROJECT>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_PROJECT storeWhse in _db.TA_PROJECT)
            {

                if (detailList.All(p => p.UID != storeWhse.UID))
                    _db.TA_PROJECT.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_PROJECT> entry;
...
//                        _db.TA_PROJECT.Attach(storeWhse);
                        break;
                    default:
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_PROJECT.Any(p => p.UID == detail.UID)))
            {
                _db.TA_PROJECT.Add(detail);
