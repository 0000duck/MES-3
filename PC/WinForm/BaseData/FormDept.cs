...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormDept: Office2007Form
    {
        private SpareEntities _db = EntitiesFactory.CreateSpareInstance();
        public FormDept()
        {
            InitializeComponent();
            bs.DataSource = _db.TA_DEPT.ToList();
...
//            grid.PrimaryGrid.ColumnAutoSizeMode = ColumnAutoSizeMode.Fill;
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_DEPT>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_DEPT storeWhse in _db.TA_DEPT)
            {
...
//                        _db.TA_DEPT.Attach(storeWhse);
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_DEPT.Any(p => p.UID == detail.UID)))
            {
                _db.TA_DEPT.Add(detail);
