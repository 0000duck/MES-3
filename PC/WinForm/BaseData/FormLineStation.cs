...
//            grid.PrimaryGrid.ColumnAutoSizeMode = ColumnAutoSizeMode.Fill;
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_LINESTATION>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_LINESTATION storeWhse in _db.TA_LINESTATION)
            {

                if (detailList.All(p => p.ID != storeWhse.ID))
                    _db.TA_LINESTATION.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_LINESTATION> entry;
...
//                        _db.TA_DEPT.Attach(storeWhse);
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_LINESTATION.Any(p => p.ID == detail.ID)))
            {
                _db.TA_LINESTATION.Add(detail);
