...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormWorkline : Office2007Form
    {
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_WORKLINE>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_WORKLINE storeWhse in _db.TA_WORKLINE)
            {

                if (detailList.All(p => p.UID != storeWhse.UID))
                    _db.TA_WORKLINE.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_WORKLINE> entry;
...
//                        _db.TA_WORKLINE.Attach(storeWhse);
                        break;
                    default:
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_WORKLINE.Any(p => p.UID == detail.UID)))
            {
                _db.TA_WORKLINE.Add(detail);
