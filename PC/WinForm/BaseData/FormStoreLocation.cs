...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormStoreLocation : Office2007Form
    {
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_STORE_LOCATION>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_STORE_LOCATION storeWhse in _db.TA_STORE_LOCATION)
            {
...
//                        _db.TA_STORE_LOCATION.Attach(storeWhse);
                        break;
                    default:
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_STORE_LOCATION.Any(p => p.UID == detail.UID)))
            {
                _db.TA_STORE_LOCATION.Add(detail);
