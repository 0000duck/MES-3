...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormUnit : Office2007Form
    {
        private SpareEntities _db = EntitiesFactory.CreateSpareInstance();
        public FormUnit()
        {
            InitializeComponent();
            bs.DataSource = _db.TA_UNIT.ToList();
        ...
        {
//            object[] list = {"工厂仓库", "第三方仓库"};
//            gridColumn5.EditorParams = list;
//
//            var listState = EnumHelper.EnumToList<DataState>();
//            gridColumn5.EditorParams = new object[] {listState};
        }
        private  void btnSave_Click(object sender, EventArgs e)
        {
            bs.EndEdit();
            var detailList = (List<TA_UNIT>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_UNIT storeWhse in _db.TA_UNIT)
            {

                if (detailList.All(p => p.UID != storeWhse.UID))
                    _db.TA_UNIT.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_UNIT> entry;
...
//                        _db.TA_UNIT.Attach(storeWhse);
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_UNIT.Any(p => p.UID == detail.UID)))
            {
                _db.TA_UNIT.Add(detail);
