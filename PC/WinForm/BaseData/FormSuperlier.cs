...
namespace ChangKeTec.Wms.WinForm.BaseData
{
    ...
    public partial class FormSuperlier : Office2007Form
    {
        private SpareEntities _db = EntitiesFactory.CreateWmsInstance();
        public FormSuperlier()
        {
            InitializeComponent();
            bs.DataSource = _db.TA_SUPERLIER.ToList();
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_SUPERLIER>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_SUPERLIER storeWhse in _db.TA_SUPERLIER)
            {

                if (detailList.All(p => p.UID != storeWhse.UID))
                    _db.TA_SUPERLIER.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_SUPERLIER> entry;
...
//                        _db.TA_SUPERLIER.Attach(storeWhse);
                        break;
                    default:
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_SUPERLIER.Any(p => p.UID == detail.UID)))
            {
                _db.TA_SUPERLIER.Add(detail);
