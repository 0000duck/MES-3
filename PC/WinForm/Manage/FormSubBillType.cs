...
namespace ChangKeTec.Wms.WinForm.Manage
{
    ...
    public partial class FormSubBillType : Office2007Form
    {
        private SpareEntities _db = EntitiesFactory.CreateSpareInstance();
        public FormSubBillType()
        {
            InitializeComponent();
            bs.DataSource = _db.TA_SUB_BILLTYPE.ToList();
        ...
        {
            bs.EndEdit();
            var detailList = (List<TA_SUB_BILLTYPE>) bs.DataSource;
            TL_BASEDATA log=null;
            
            foreach (TA_SUB_BILLTYPE storeWhse in _db.TA_SUB_BILLTYPE)
            {

                if (detailList.All(p => p.UID != storeWhse.UID))
                    _db.TA_SUB_BILLTYPE.Remove(storeWhse);
                
                OperateType logType;
                string oldValue, newValue;
                DbEntityEntry<TA_SUB_BILLTYPE> entry;
...
//                        _db.TA_SUB_BILLTYPE.Attach(storeWhse);
                        break;
                    default:
            ...
            foreach (var detail in detailList.Where(detail => !_db.TA_SUB_BILLTYPE.Any(p => p.BillType == detail.BillType)))
            {
                _db.TA_SUB_BILLTYPE.Add(detail);
